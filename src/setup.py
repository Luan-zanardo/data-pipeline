...
def populate_source_database_if_empty():
    ...
    try:
        ...
        print("ERRO: Não foi possível conectar ao banco de dados de origem.")
        raise
    ...
    if table_exists:
        ...
        if count > 0:
            print(f"O banco de dados de origem já está populado com {count} registros em 'pedidos'. Nenhuma ação necessária.")
            cur.close()
            conn.close()
            return

    print("Banco de dados vazio. Iniciando a criação de tabelas e população...")
    ...
    # ... (O restante da lógica de geração de dados é idêntica) ...
